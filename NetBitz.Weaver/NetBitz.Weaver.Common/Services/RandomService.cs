...
namespace NetBitz.Weaver.Common.Services
{
    /// <summary>
	///     A seeded SHA256 PRNG. Adapted from ConfuserEx
	/// </summary>
	public class RandomGenerator
    {
        ...
        internal static byte[] Seed(string seed)
        {
            ...
            if (!string.IsNullOrEmpty(seed))
                ret = RandomServiceUtil.SHA256(Encoding.UTF8.GetBytes(seed));
            else
                ret = RandomServiceUtil.SHA256(Guid.NewGuid().ToByteArray());

            for (int i = 0; i < 32; i++)
            {
                ret[i] *= primes[i % primes.Length];
                ret = RandomServiceUtil.SHA256(ret);
            }
            return ret;
        }
    ...
    internal class RandomService : IRandomService
    {
        ...
        public RandomGenerator GetRandomGenerator(string id)
        {
            ...
            byte[] newSeed = seed;
            byte[] idHash = RandomServiceUtil.SHA256(Encoding.UTF8.GetBytes(id));
            for (int i = 0; i < 32; i++)
                newSeed[i] ^= idHash[i];
            return new RandomGenerator(RandomServiceUtil.SHA256(newSeed));
        }
    }
